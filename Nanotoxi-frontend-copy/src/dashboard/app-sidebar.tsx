...
type Section = "overview" | "incidents" | "performance" | "toxicity" | "errors" | "services" | "oncall" | "sla" | "postmortems" | "settings";
...
const favorites: NavItem[] = [
  { id: "incidents", label: "Data & Requests", icon: Database },
  { id: "toxicity", label: "Toxicity Analysis", icon: PieChart },
];

const mainMenu: NavItem[] = [
  { id: "overview", label: "Dashboard", icon: LayoutDashboard },
  { id: "incidents", label: "Data & Requests", icon: Database, badge: 3, badgeColor: "red" },
  { id: "toxicity", label: "Toxicity Analysis", icon: PieChart },
  { id: "performance", label: "Prediction History", icon: Activity },
  { id: "sla", label: "SLA & Uptime", icon: Shield },
  { id: "services", label: "Forms Console", icon: Server },
        ...
        <div className="flex items-center justify-between gap-1 px-2 py-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex flex-1 items-center gap-3 overflow-hidden rounded-xl p-2 hover:bg-muted/60 transition-colors text-left outline-none group">
                <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0 border border-border group-hover:border-primary/30 transition-colors">
                  <span className="text-primary text-xs font-semibold">{user?.name ? user.name.substring(0, 2).toUpperCase() : "JD"}</span>
                </div>
                <div className="flex flex-col min-w-0">
                  <p className="text-sm font-semibold text-foreground truncate leading-none">{user?.name || "John Doe"}</p>
                  <p className="text-[10px] text-muted-foreground truncate mt-1">{user?.designation || "SRE Lead"}</p>
                </div>
              </button>
            </DropdownMenuTrigger>
